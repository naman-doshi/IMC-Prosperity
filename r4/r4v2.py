...
class Trader:
    ...
    def run(self, state: TradingState) -> dict[Symbol, list[Order]]:
        ...
        if basketSpread > self.target and self.target == 0.9944 + plusMinus:
            self.shortingBasket = 'COMPOSITE'
            self.buildingBasket = 'PICNIC_BASKET' 
            self.target = 0.9944 - plusMinus
        elif basketSpread < self.target and self.target == 0.9944 - plusMinus:
